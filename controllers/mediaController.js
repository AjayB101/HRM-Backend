...
exports.create = async (req, res) => {
  ...
  let videosUrls = [];
  let imageUrl = '';
  let durations = req.body.durations || []; // Handle undefined durations

  if (Array.isArray(req.files.videos) && req.files.videos.length > 0) {
    for (let i = 0; i < req.files.videos.length; i++) {
      const video = req.files.videos[i];
      const duration = durations[i] || 0; // Use 0 if duration is undefined
      const result = await cloudinary.uploader.upload(video.path, {
        folder: 'Videos',
        resource_type: 'video', // Set resource_type to 'video'
        use_filename: true,
        overwrite: true,
        metadata: { duration: duration } // Add duration metadata
      }).catch(error => {
